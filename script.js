...
class TabNow {
    constructor() {
        ...
        this.authBtnText = this.authBtn?.querySelector('.auth-btn-text');
    ...
    setupAuth() {
        ...
        if (typeof initializeFirebase === 'function' && isFirebaseConfigured()) {
            initializeFirebase();
            ...
            SyncService.addSyncListener((state) => {
                this.updateSyncStatus(state);
            });
        } else {
            // Firebase not configured - hide auth button
            if (this.authBtn) {
        ...
        // Auth button click - show modal or dropdown
        if (this.authBtn) {
    ...
    handleAuthStateChange(user) {
        ...
        if (user) {
            ...
            this.realtimeSyncUnsubscribe = SyncService.setupRealtimeSync((data) => {
    ...
    updateAuthUI(isSignedIn) {
        ...
        if (isSignedIn && this.currentUser) {
            ...
            if (photoURL) {
                this.authBtn.innerHTML = `<img src="${photoURL}" alt="Profile" class="user-avatar">`;
            } else {
                this.authBtn.innerHTML = `<span class="user-initial">${initial}</span>`;
            }
            this.authBtn.classList.add('signed-in');
            this.authBtn.title = `Signed in as ${displayName}`;
            ...
            // Show sign in button
            this.authBtn.innerHTML = '<span class="auth-btn-text">Sign In</span>';
            this.authBtn.classList.remove('signed-in');
            this.authBtn.title = 'Sign in to sync';
        ...
        if (this.syncStatus) {
            this.syncStatus.style.display = isSignedIn ? 'flex' : 'none';
        }
    }
    ...
    updateSyncStatus(state) {
        ...
        switch (state) {
            case 'syncing':
                ...
                this.syncStatus.title = 'Syncing...';
                break;
            case 'synced':
                this.syncStatus.classList.add('synced');
                this.syncStatus.title = 'All changes synced';
                break;
            case 'error':
                this.syncStatus.classList.add('error');
                this.syncStatus.title = 'Sync error - click to retry';
                break;
            default:
                this.syncStatus.title = 'Sync status';
        }
    }
    ...
    updateAuthModalUI() {
        if (this.isSignUpMode) {
            if (this.authModalTitle) this.authModalTitle.textContent = 'Create Account';
            if (this.authSubmitBtn) this.authSubmitBtn.textContent = 'Sign Up';
            if (this.authSwitchText) this.authSwitchText.textContent = 'Already have an account?';
            if (this.authSwitchBtn) this.authSwitchBtn.textContent = 'Sign In';
        } else {
            if (this.authModalTitle) this.authModalTitle.textContent = 'Sign In';
            if (this.authSubmitBtn) this.authSubmitBtn.textContent = 'Sign In';
            if (this.authSwitchText) this.authSwitchText.textContent = "Don't have an account?";
            if (this.authSwitchBtn) this.authSwitchBtn.textContent = 'Sign Up';
        }
        if (this.authError) this.authError.textContent = '';
    }
    ...
    setAuthLoading(isLoading) {
        if (this.authSubmitBtn) {
            this.authSubmitBtn.disabled = isLoading;
            this.authSubmitBtn.textContent = isLoading ? 'Please wait...' : (this.isSignUpMode ? 'Sign Up' : 'Sign In');
        }
        if (this.googleSignInBtn) {
            this.googleSignInBtn.disabled = isLoading;
        }
    }
    ...
    async loadDataFromCloud() {
        ...
        try {
            const cloudData = await SyncService.loadUserData();
            
            if (cloudData) {
                // Merge cloud data with local data (cloud wins for conflicts)
                if (cloudData.notes?.content) {
                    this.notepadEditor.innerHTML = cloudData.notes.content;
                    localStorage.setItem('tabNowNotes', cloudData.notes.content);
                }
                
                if (cloudData.todos?.items) {
                    this.todos = cloudData.todos.items;
                    localStorage.setItem('tabNowTodos', JSON.stringify(this.todos));
                    this.renderTodos();
                }
                
                if (cloudData.reminders?.items) {
                    this.reminders = cloudData.reminders.items;
                    localStorage.setItem('tabNowReminders', JSON.stringify(this.reminders));
                    this.renderReminders();
                }
                ...
                if (cloudData.preferences) {
                    if (cloudData.preferences.theme) {
                        this.setTheme(cloudData.preferences.theme);
                    }
                    if (cloudData.preferences.tempUnit) {
                        this.temperatureUnit = cloudData.preferences.tempUnit;
                        localStorage.setItem('tabNowTempUnit', cloudData.preferences.tempUnit);
                    }
                    if (cloudData.preferences.clockLocations) {
                        this.additionalClockLocations = cloudData.preferences.clockLocations;
                        localStorage.setItem('tabNowAdditionalClockLocations', JSON.stringify(cloudData.preferences.clockLocations));
                        this.renderAdditionalLocations();
                    }
                }
    ...
    handleRemoteDataChange(data) {
        ...
        if (data.notes?.content && data.notes.content !== this.notepadEditor.innerHTML) {
            this.notepadEditor.innerHTML = data.notes.content;
            localStorage.setItem('tabNowNotes', data.notes.content);
        }
        
        if (data.todos?.items) {
            const newTodos = JSON.stringify(data.todos.items);
            const currentTodos = JSON.stringify(this.todos);
            if (newTodos !== currentTodos) {
                this.todos = data.todos.items;
                localStorage.setItem('tabNowTodos', JSON.stringify(this.todos));
                this.renderTodos();
            }
        }
        
        if (data.reminders?.items) {
            const newReminders = JSON.stringify(data.reminders.items);
            const currentReminders = JSON.stringify(this.reminders);
            if (newReminders !== currentReminders) {
                this.reminders = data.reminders.items;
                localStorage.setItem('tabNowReminders', JSON.stringify(this.reminders));
                this.renderReminders();
            }
        }
    ...
    async syncAllDataToCloud() {
        ...
        const data = {
            notes: this.notepadEditor.innerHTML,
            todos: this.todos,
            reminders: this.reminders,
            theme: document.documentElement.getAttribute('data-theme'),
            tempUnit: this.temperatureUnit,
            clockLocations: this.additionalClockLocations,
    ...
    saveDataImmediately() {
        ...
        // Sync to Firebase cloud if user is signed in
        if (this.currentUser && typeof SyncService !== 'undefined') {
